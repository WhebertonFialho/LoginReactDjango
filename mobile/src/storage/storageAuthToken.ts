...
export async function storageAuthTokenSalvar({token, refresh_token} : StorageAuthTokenProps) {
  await AsyncStorage.setItem(AUTH_STORAGE, JSON.stringify({ token, refresh_token }));
}

export async function storageAuthTokenBuscar() {
...
export async function storageAuthTokenRemover() {
